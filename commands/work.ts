...
export default {
	...
	run: async function work(message, _args, _users, user, _items) {
		...
		if (user.data.storage.work_cooldown &&
			...
			return message.reply(`woah, chill out, no one wants to work as a minimum-wage slave *that* much. come back at ${user.data.storage.work_cooldown} millisecond unix`);
 ...
 you can work again at ${user.data.storage.work_cooldown} millisecond unix`);
	}
} as Command;
